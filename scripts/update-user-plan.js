...
if (!userEmail || !planKey) {
  console.error('Usage: node scripts/update-user-plan.js <user_email> <plan_key>');
  console.error('Example: node scripts/update-user-plan.js user@example.com basic');
  console.error('Available plans: free, basic, premium');
  process.exit(1);
}

// Validate plan key
if (!['free', 'basic', 'premium'].includes(planKey)) {
  console.error('Error: Plan key must be one of: free, basic, premium');
  process.exit(1);
}
...
async function updateUserPlan() {
  try {
    ...
    const { stdout: usersOutput } = await execPromise('npx convex run users:getAllUsers --prod');
    ...
    // Get plan details
    const { stdout: plansOutput } = await execPromise('npx convex run plans:getPlans --prod');
    ...
    console.log(`- Max Session Duration: ${plan.maxSessionDurationMinutes} minutes`);
    ...
    const { stdout: updateOutput } = await execPromise('npx convex run temp:updatePlanTemp --prod');
    ...
    if (updateResult.success) {
      ...
      console.log(`- Max Session Duration: ${updateResult.maxSessionDuration} minutes`);
    } else {

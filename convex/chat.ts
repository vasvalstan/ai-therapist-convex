...
function normalizeHumeEvent(event: any): Message {
  return {
    ...
    metadata:
      event.metadata ||
      (event.chatId && event.chatGroupId
        ? {
...
async function checkUserChatAccess(ctx: QueryCtx, userId: string) {
  ...
  // Get user's plan
  const planKey = user.currentPlanKey;

  // If user has no plan, they need to choose one
  if (!planKey) {
    return {
      hasAccess: false,
      reason:
        "Please choose a subscription plan to start chatting with our AI therapist.",
  ...
  if (!plan) {
    return {
      hasAccess: false,
      reason:
        "Your current plan is no longer available. Please choose a new subscription plan.",
      upgradeRequired: true,
...
export const updateUserRemainingMinutes = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(
      `Updating minutes for user ${userId}, duration: ${args.sessionDurationMinutes} minutes`
    );
    ...
    const newMinutesRemaining = Math.max(
      0,
      currentMinutesRemaining - minutesToDeduct
    );

    console.log(
      `User ${userId} minutes: ${currentMinutesRemaining} -> ${newMinutesRemaining} (deducted ${minutesToDeduct}, rounded up from ${args.sessionDurationMinutes})`
    );
    console.log(
      `User plan: ${planKey}, total allowed: ${user.totalMinutesAllowed || 0}`
    );
...
function validateEventType(
  type: string
): "USER_MESSAGE" | "AGENT_MESSAGE" | "SYSTEM_MESSAGE" | "CHAT_METADATA" {
  const validTypes = [
    "USER_MESSAGE",
    "AGENT_MESSAGE",
    "SYSTEM_MESSAGE",
...
function messageToEvent(
  ...
  return {
    ...
    emotionFeatures: message.emotionFeatures
      ? JSON.stringify(message.emotionFeatures)
      : undefined,
...
export const addMessageToSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const role =
      typeof args.message.role === "string" &&
...
export const getAllChatHistory = query({
  handler: async (ctx) => {
    ...
    return await ctx.db.query("chatHistory").collect();
  },
});
...
export const updateHumeChatIds = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!chatSession) {
      throw new Error(
        "Chat session not found - tried sessionId, chatId, and recent sessions"
      );
    }
...
export const saveConversationTranscript = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!chatSession && args.chatId) {
      console.log(
        `⚠️ Session not found by sessionId, trying chatId: ${args.chatId}`
      );
      chatSession = await ctx.db
        .query("chatHistory")
        .filter((q) => q.eq(q.field("userId"), userId))
      ...
      if (chatSession) {
        console.log(
          `✅ Found session by chatId: ${args.chatId} -> sessionId: ${chatSession.sessionId}`
        );
      }
    }
    ...
    if (!chatSession && args.chatGroupId) {
      console.log(
        `⚠️ Session not found by chatId, trying chatGroupId: ${args.chatGroupId}`
      );
      chatSession = await ctx.db
        .query("chatHistory")
        .filter((q) => q.eq(q.field("userId"), userId))
      ...
      if (chatSession) {
        console.log(
          `✅ Found session by chatGroupId: ${args.chatGroupId} -> sessionId: ${chatSession.sessionId}`
        );
      }
    }
    ...
    if (!chatSession && args.events.length > 0 && args.events[0].chatId) {
      ...
      console.log(
        `⚠️ Session not found by params, trying first event chatId: ${firstEventChatId}`
      );
      ...
      if (chatSession) {
        console.log(
          `✅ Found session by first event chatId: ${firstEventChatId} -> sessionId: ${chatSession.sessionId}`
        );
      }
    }
    ...
    if (!chatSession) {
      ...
      if (chatSession) {
        console.log(
          `✅ Using most recent session as fallback: ${chatSession.sessionId}`
        );
      } else {
        console.error(`❌ No session found for user, cannot save transcript`);
        throw new Error(
          "Chat session not found - no sessions available for this user"
        );
      }
    }
    ...
    console.log(
      `✅ Successfully saved transcript to session: ${chatSession.sessionId}`
    );
...
export const updateMetadata = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(
      `📝 Updating ${messages.length} messages and ${events.length} events`
    );
...
export const updateAndVerifyMetadata = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!sessionBefore) {
      console.log(
        `⚠️ Session not found by sessionId ${args.sessionId}, trying chatId ${args.chatId}`
      );
      sessionBefore = await ctx.db
        .query("chatHistory")
        .filter((q) => q.eq(q.field("userId"), userId))
      ...
      if (sessionBefore) {
        console.log(
          `✅ Found session using chatId instead: ${sessionBefore.sessionId}`
        );
      }
    }
    ...
    if (!sessionBefore) {
      ...
      if (sessionBefore) {
        console.log(
          `✅ Found session using chatGroupId instead: ${sessionBefore.sessionId}`
        );
      }
    }
    ...
    if (!sessionBefore) {
      ...
      if (sessionBefore) {
        console.log(
          `✅ Using most recent session as fallback: ${sessionBefore.sessionId}`
        );
      }
    }
...
export const createSessionWithHumeIds = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(
      `Creating new session with Hume IDs: chatId=${args.chatId}, chatGroupId=${args.chatGroupId}`
    );
...
export const addSystemMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(session._id, {
      ...
      events: [
        ...(session.events || []),
        messageToEvent(message, session.chatId, session.chatGroupId),
      ],
      updatedAt: Date.now(),
    });
